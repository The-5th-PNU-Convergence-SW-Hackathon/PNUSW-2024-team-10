...
void showSignOutConfirmDialog(BuildContext context) async {
  if (context.mounted) {
    showDialog(
      context: context,
      builder: (BuildContext context) {
        return const SignOutConfirmDialog();
      },
    );
  }
}
...
class SignOutConfirmDialog extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return AlertDialog(
      backgroundColor: Theme.of(context).colorScheme.surfaceBright,
      title: Text(l10n.moreEtcSignOut),
      content: Text(l10n.moreEtcSignOutMessage),
      actions: [
        TextButton(
          onPressed: () {
            Navigator.pop(context);
